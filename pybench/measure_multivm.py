...
class PktgenMultiVMTest(AbstractBenchTest):
    ...
    def pre_initial_cleanup(host: Host, qemu_pid, pktgen_pid):
        ...
        for pid, label in [(qemu_pid, "qemu"), (pktgen_pid, "pktgen")]:
            if pid is not None:
                children = PktgenMultiVMTest._get_child_pids(host, pid)
                if children:
        ...
        if all_children:
            for attempt in range(6):
                survivors = PktgenMultiVMTest._check_survivors(host, all_children)
                if not survivors:
                    break
                warning(f"Retry {attempt + 1}/6: killing {len(survivors)} surviving children")
