...
def setup_parser() -> ArgumentParser:
    ...
    parser.add_argument('--extremes-only',
                        action='store_true',
                        help='Only test extreme parameter values (min/max) for a quick smoke test of all code paths.',
                        )
...
class AbstractBenchTest(ABC):
    ...
    @classmethod
    def filter_extremes(cls: Type[A], tests: List[A], test_parameters: List[str]) -> List[A]:
        """
        Filter tests to only include those at parameter extremes (min/max for numeric params).
        For sparse test matrices where an extreme combination doesn't exist,
        snaps each numeric param to the closest available value.
        """
        if len(tests) == 0:
            return tests

        test_mat = cls.test_matrix(tests)
        loop_matrix = { key: value for (key, value) in test_mat.items() if key in test_parameters }
        ...
        for iteration_parameters in potential_iterations:
            iteration_tests = [ t for t in tests if all(t.__dict__[k] == v for k, v in iteration_parameters.items()) ]
