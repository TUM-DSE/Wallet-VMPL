{ buildLinux, fetchFromGitHub, kernelVariantName ? "default", ... }@args:
let
  ...
  kernelVariants = {
    inherit coconut_svsm coconut_svsm_6_8 coconut_svsm_with_reflectvc_vmpl
      coconut_svsm_with_restricted_interrupt coconut_svsm_6_11
      svsm_preview_hv_1 svsm_preview_hv_2;
    default = coconut_svsm_with_reflectvc_vmpl;
  };

  snp_kernel = kernelVariants.${kernelVariantName};
in
with snp_kernel;
buildLinux (args // rec {
  inherit version;
