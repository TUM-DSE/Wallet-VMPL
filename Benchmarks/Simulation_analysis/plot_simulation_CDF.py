...
def parallel_plot_node_size(args):
    ...
    # Generate plot
    if (value_type == "delays"):
      title = f"(a) VNF orchestration latency CDF ({node_size/PER_SERVER_CPUS} servers)    "
    elif (value_type == "slowdowns"):
      title = f"(b) Per function slowdown CDF ({node_size/PER_SERVER_CPUS} servers)"
